const btnAdd = document.querySelector('#button-add') as HTMLButtonElement;
const btnRemove = document.querySelector('#button-remove') as HTMLButtonElement;
const btnPriority = document.querySelector('#priority-btn') as HTMLButtonElement;
const app = document.querySelector('.app-todo') as HTMLElement;
const container = document.querySelector('.content-todo') as HTMLElement;
const btnAddNote = document.querySelector('.add-note') as HTMLButtonElement;
const btnRemoveNote = document.querySelector('.remove-note') as HTMLButtonElement;
...
let flag = false;
const user = {
  task: [] as HTMLDivElement[],
  check: [] as HTMLInputElement[],
  textarea: [] as HTMLTextAreaElement[],
};
const sideNote = {
  note: [] as HTMLDivElement[],
  text: [] as HTMLTextAreaElement[],
  checkNote: [] as HTMLInputElement[],
};
...
let number = 0;
const priority = (prio: NodeListOf<HTMLDivElement>) => {
  prio.forEach((item) =>
    item.addEventListener('click', () => {
      item.style.borderRight = `5px solid ${colorArray[number++]}`;
      if (number === colorArray.length) number = 0;
    })
  );
};
...
const catchElement = (e: MouseEvent) => {
  flag = !flag;
