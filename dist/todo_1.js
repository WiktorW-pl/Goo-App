...
const buildTask = () =>{
    ...
    prio = document.querySelectorAll('.color')
    priority(prio);
}
...
const moveElement = (e) =>{
    sideNote.note.forEach(item => {
        if(flag==true && item.classList.contains('done')==false){
            x = Math.floor((e.clientX*100)/innerWidth);
            y = Math.floor((e.clientY*100)/innerHeight);
            item.style.top = y -7 + "%";
            item.style.left = x -5 + "%";
